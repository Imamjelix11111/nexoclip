...
export function createOpenAIImageAdapter({ apiKey, baseUrl = 'https://api.openai.com/v1', fetch: fetchImpl = globalThis.fetch } = {}) {
  return { async generate({ model, prompt, size, quality }) {
    let response;
    try { response = await fetchImpl(`${String(baseUrl).replace(/\/+$/, '')}/images/generations`, { method: 'POST', headers: { Authorization: `Bearer ${apiKey}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model, prompt, ...(size ? { size } : {}), ...(quality ? { quality } : {}) }) }); } catch (error) { throw transientError('openai', error); }
    if (!response.ok) throw transientError('openai', response);
...
export function createGoogleImageAdapter({ apiKey, baseUrl = 'https://generativelanguage.googleapis.com/v1beta', fetch: fetchImpl = globalThis.fetch } = {}) {
  return { async generate({ model, prompt, aspectRatio, resolution }) {
    let response;
    try { response = await fetchImpl(`${String(baseUrl).replace(/\/+$/, '')}/models/${encodeURIComponent(model)}:generateContent?key=${encodeURIComponent(apiKey)}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }], generationConfig: { responseModalities: ['TEXT', 'IMAGE'], ...((aspectRatio || resolution) ? { imageConfig: { ...(aspectRatio ? { aspectRatio } : {}), ...(resolution ? { imageSize: resolution } : {}) } } : {}) } }) }); } catch (error) { throw transientError('google', error); }
    if (!response.ok) {
