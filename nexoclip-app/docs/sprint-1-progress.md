...
## Spite realtime CRDT Task 14 implementation note

- What changed: Added a pure local-presence snapshot seam that derives the current editing node from `document.activeElement`, then used it to republish selection/editing whenever the presence controller is created or an awareness reconnect leaves local presence fields missing. Realtime presence projection now runs once in the workspace on the shared 1 Hz lock-expiry snapshot, the overlay consumes projected peers directly, and lock membership is memoized by membership key so unchanged locks do not churn `sceneNodes` or resend selection.
- Files: `services/spite/lib/realtime/presence.ts`, `services/spite/lib/realtime/presence.test.ts`, `services/spite/components/canvas/canvas-workspace.tsx`, `services/spite/components/canvas/realtime-presence.tsx`, `docs/sprint-1-progress.md`.
- Tests: `cd services/spite && rtk npx tsx --test lib/realtime/presence.test.ts` — 8 passed, 0 failed after the expected red run for the missing local-presence snapshot helpers.
- Verification: `cd services/spite && rtk npm run test:realtime` — 78 passed, 0 failed, 5 skipped. `cd .. && rtk npx tsc -p services/spite/tsconfig.json --noEmit` — passed with no TypeScript errors. Realtime test runs emitted the existing experimental `localStorage` warning from the current harness; no new warning class was introduced. Notion MCP/task-page sync was unavailable in this session, so only the local progress log was updated.
- Blockers: No code blocker remains for this task. Notion MCP was unavailable, so no matching remote task record could be updated.
- Follow-up: If a future task adds canvas component tests, add a focused integration case that exercises controller recreation through the actual workspace hook/component boundary rather than only the extracted presence seam.
