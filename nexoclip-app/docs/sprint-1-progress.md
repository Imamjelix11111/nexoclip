...
## Sprint 1 — Spite realtime CRDT Round 1 Task 9 implementation note

- What changed: Tightened the realtime lifecycle regression to correlate websocket authorization and sync ordering per connection instead of by global event index, added connection-scoped lifecycle instrumentation on server events, preserved the unauthorized assertions that failed connections never hydrate or receive sync state, and added a direct `GET /healthz` coverage check that proves the endpoint stays unauthenticated and side-effect free.
- Files: `services/spite/realtime/server.ts`, `services/spite/realtime/server-core.test.ts`, `docs/sprint-1-progress.md`.
- Tests: `cd services/spite && npm exec tsx -- --test realtime/server-core.test.ts` — 4 passed, 0 failed after the expected red run when the new per-connection lifecycle assertion found missing connection IDs.
- Verification: `cd services/spite && npm exec tsc -- --noEmit -p tsconfig.json` — passed. Final diff review and `rtk git diff --check` were run before commit; no unrelated files were changed.
- Blockers: Notion MCP was unavailable, so no task-board synchronization was possible. The focused realtime test still emits the existing Node experimental `localStorage` warning from the provider/runtime stack; no runtime behavior change was made for that warning.
- Follow-up: If more lifecycle probes are needed later, keep `connectionId` as the canonical test correlation field and leave the legacy `socketId` metadata in place for compatibility.
