...
test('duplicate, canvas read, and folders fail closed for non-owners', async () => {
  const fixture = createFakeSqlFixture()
  fixture.seedProject({ id: OWNER_PROJECT_ID, userid: OWNER_ID, name: 'Mine' })
  ...
  const foldersResponse = await folders.GET(makeRequest(`http://spite.local/api/folders?projectId=${OWNER_PROJECT_ID}`) as any)

  assert.equal(duplicateResponse.status, 404)
  assert.equal(canvasResponse.status, 404)
  assert.equal(foldersResponse.status, 404)
})
