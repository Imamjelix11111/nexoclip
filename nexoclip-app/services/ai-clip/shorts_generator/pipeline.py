...
def _run_local(
    ...
    source_path = download_youtube_local(youtube_url, fmt=download_format, out_dir=out_dir)
    ...
    highlights_result = get_highlights(transcript, num_clips=num_clips, llm_fn=call_local_llm, topic_hint=topic_hint)
    ...
    shorts = crop_highlights_local(source_path, top, aspect_ratio=aspect_ratio, out_dir=out_dir)
...
def _run_api(
    ...
    highlights_result = get_highlights(transcript, num_clips=num_clips, llm_fn=call_muapi_llm, topic_hint=topic_hint)
    all_highlights: List[Dict] = highlights_result.get("highlights", [])
    if not all_highlights:
        raise RuntimeError("Highlight generator returned zero clips.")
...
def generate_shorts(
    ...
    if mode == "local":
        return _run_local(youtube_url, num_clips, aspect_ratio, download_format, language, topic_hint=topic_hint, out_dir=out_dir)
    if mode == "api":
        return _run_api(youtube_url, num_clips, aspect_ratio, download_format, language, topic_hint=topic_hint)
