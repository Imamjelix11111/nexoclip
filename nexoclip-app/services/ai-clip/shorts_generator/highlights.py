...
def call_highlight_api(
    ...
    topic_hint_instruction = (
        f'- The user specifically wants clips about: "{topic_hint}" — prioritize highlights matching this over the general virality signals above'
        if topic_hint else ""
    )
    system = HIGHLIGHT_SYSTEM_PROMPT.format(
        ...
        topic_hint_instruction=topic_hint_instruction,
    )
...
def get_highlights(
    ...
    mode passes in a local LLM-backed callable. `topic_hint` is an optional
    free-text steer ("focus on the funny moments", "the part about pricing")
    that gets folded into the highlight-selection prompt.
    """
    ...
    if duration >= LONG_VIDEO_THRESHOLD:
        ...
        for i, chunk in enumerate(chunks):
            ...
            result = call_highlight_api(text, content_info, chunk["duration"], num_clips=num_clips, is_chunk=True, llm_fn=llm_fn, topic_hint=topic_hint)
        ...
        result = call_highlight_api(text, content_info, duration, num_clips=num_clips, llm_fn=llm_fn, topic_hint=topic_hint)
