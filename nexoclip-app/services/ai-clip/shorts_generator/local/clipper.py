...
def _reframe_vertical(in_path: str, out_path: str, aspect_ratio: str) -> str:
    ...
    # Encode straight to H.264 via a piped ffmpeg process instead of cv2.VideoWriter
    # (fourcc "mp4v" — a lossy MPEG-4 Part 2 pass with no bitrate control that then
    # had to be re-encoded *again* for browser playback). One encode pass, not two.
    ...
    if returncode != 0:
        raise RuntimeError(f"ffmpeg raw-frame encode failed with exit code {returncode}")

    # silent_path is already H.264 — just copy it in while adding back the audio
    # from the cut clip, no second re-encode.
    cmd = [
        "ffmpeg", "-y", "-loglevel", "error",
        "-i", silent_path,
        "-i", in_path,
