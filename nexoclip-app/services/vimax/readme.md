...
For Nexoclip production deployments, the service expects the authenticated proxy to provide `x-nexoclip-tenant`. Tenant data is isolated under `.tenants/<userId>/`; do not expose port `4173` publicly. The standalone development server may use the `default` tenant when `NODE_ENV` is not `production`.
...
In Docker, Nexoclip should connect to `http://ai-storyboard:4173` over the private compose network. Provider configuration is global for the MVP; do not store API keys inside tenant directories.
