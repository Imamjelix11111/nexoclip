...
def test_executor_sanitizes_absolute_paths_and_serializes_workspace_session(monkeypatch, tmp_path):
    ...
    class FakeAdapters:
        ...
        async def vimax_render_video(self, args, runtime):
            ...
            runtime.emit_progress("rendering", metadata={"outside": Path("/private/progress.mp4")})
            ...
            return ToolResult(
                ...
                True,
                "done",
                {"inside": self.workspace_root / "clip.mp4", "outside": "/private/result.mp4"},
            )
    ...
    assert results[0]["result"] == {"inside": "workspace-1/clip.mp4", "outside": "result.mp4"}
    assert results[0]["progress"][0]["progress"]["metadata"] == {"outside": "progress.mp4"}
