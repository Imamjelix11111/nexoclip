...
// Monotonic counter so ids stay unique even when several events land in the
// same millisecond (stderr is flushed line-by-line, so Date.now() alone collides).
let idSeq = 0;
const uniqueId = (prefix: string) => `${prefix}-${Date.now()}-${idSeq++}`;
...
export function appendLocalUser(state: ChatState, text: string): ChatState {
  return {
    ...state,
    busy: true,
    messages: [...state.messages, {id: uniqueId('local-user'), role: 'user', text}],
  };
}
  ...
  switch (event.type) {
    ...
    case 'error':
      return {
        ...state,
        busy: false,
        messages: [...state.messages, {id: uniqueId(`error-${turnId}`), role: 'error', text: event.message || 'Unknown agent error'}],
      };
    case 'done': {
...
function upsertLatestToolActivity(state: ChatState, tool: string, update: Omit<Message, 'id'>): ChatState {
  ...
  return {...state, messages: [...messages, {id: uniqueId(`activity-${tool}`), ...update}]};
}
